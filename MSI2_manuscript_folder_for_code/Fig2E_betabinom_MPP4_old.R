...
df1 <- read.csv( "prepare_betabinom_input/Mouse_HSPC_MPP4-unique_betabinom_input.csv" )
df1 <- df1 %>% select(-"X")
...
lrt.res <- mclapply( 1:nrow(df1), function( i ){
       ...
       fit1 <- mle.custom.h1( ref[ mpp4.adar.index ], alt[ mpp4.adar.index ],
                              ref[ -mpp4.adar.index ], alt[ -mpp4.adar.index ] )
...
saveRDS(lrt.res, "betabinom/lrt.res_HSPC_MPP4-unique_vs_others.RDS")

# Additional columns
alt.freq <- alt.counts / ( ref.counts + alt.counts )
df.stats <- data.frame(diff.frequency = rowMeans(alt.freq[, mpp4.adar.index ], na.rm=T) -
                         rowMeans(alt.freq[,-mpp4.adar.index],na.rm=T),
...
write.csv(df.stats, "betabinom/df.stats_HSPC_MPP4-unique_vs_others.csv", row.names = FALSE)

rowmask <- rowSums( alt.counts != 0 ) > 1 & # TRUE/FALSE on which row has sum of alt.counts > 1 (Karen)
( rowMeans( ref.counts[ mpp4.adar.index ] ) + rowMeans( alt.counts[ mpp4.adar.index ] ) >= 5 ) & 
...
write.csv(df1, "betabinom/Mouse_HSPC_MPP4-unique_vs_others_snp_counts_significance.csv", row.names = FALSE)

# Add gene FPKM and # edits
df1 <- read.csv("betabinom/Mouse_HSPC_MPP4-unique_vs_others_snp_counts_significance.csv")
...
write.csv(df1, "betabinom/Mouse_HSPC_MPP4-unique_vs_others_snp_counts_significance_fpkm.csv", row.names = FALSE)

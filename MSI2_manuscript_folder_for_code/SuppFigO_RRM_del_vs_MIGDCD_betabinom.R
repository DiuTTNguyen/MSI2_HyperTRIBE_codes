...
# setwd("/Users/karen/mount/chuk/MSI2_RRM_deletion_and_5_mutations/Project_08269_F/RRM_deletion_vs_MIG/")

df1 <- read.xlsx( "MOLM13_MSI2_RRM_deletion_vs_MIG_snp_counts.xlsx" )
...
mut.index <- which( grepl("deletion", colnames(ref.counts)) )
...
df.stats <- data.frame(diff.frequency = rowMeans(alt.freq[, mut.index ], na.rm=T) - 
                       ...
                       rrm.deletion.frequency = rowMeans(alt.freq[,mut.index], na.rm=T),
...
write.csv(df1, "MOLM13_RRM_deletion_vs_MIGDCD_snp_counts_significance.csv", row.names = FALSE)

# Add gene FPKM and # edits
gene.num.edits <- table( df1$entrez.id )
se <- readRDS( "rnaseq_read_count_entrez_id.rds" )
fpkm.mat <- fpkm( DESeqDataSet( se, ~1 ) ) # originally ~condition but R gives error cause it was never defined, but ~1 means there is no design -Karen

mut.index <- which( grepl("deletion", colnames(fpkm.mat)) )
dcd.index <- which( grepl("DCD", colnames(fpkm.mat)) )
mig.index <- which( grepl("MIG", colnames(fpkm.mat)) )

fpkm.mat <- data.frame( rrm.deletion.fpkm = rowMeans( fpkm.mat[,mut.index]),
...
write.csv(df1, "MOLM13_RRM_deletion_vs_MIGDCD_snp_counts_significance_fpkm.csv", row.names = FALSE)
